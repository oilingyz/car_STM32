...
int mov(float v2,float v4)
{
	...
	pid_mov_v2.ki=0.5;
	pid_mov_v2.kd=0;
	pid_mov_v2.max=85-MOTO2;
	pid_mov_v2.min=-85+MOTO2;
	...
	pid_mov_v4.ki=0.5;
	pid_mov_v4.kd=0;
	pid_mov_v4.max=85-MOTO4;
	pid_mov_v4.min=-85+MOTO4;
...
int xgo(float x)
{
	...
	pid_xmov.kp=5                                                                                                                                     ;
	pid_xmov.max=MAX_SPEED;
	pid_xmov.min=-MAX_SPEED;
	...
	if(LEN1>6 && x==1) enstop=1;  //LEN1停车
	if(LEN2>3 && x==2) enstop=1;  //LEN2停车
...
//				mov(MAX_SPEED-pid_xgo.output, MAX_SPEED+pid_xgo.output);
		}
	}
	else
	{
		//float v=(float)encoder.int_v2*0.8f;
...
int xun(float a)
{
	...
	pid_xun_k.cx=u2[1];
	pid_xun_k.watch_dog=30;

	pid_xun_r.kp=-1;
	pid_xun_r.target=XUN_MIDDLE_VALUE;
	pid_xun_r.cx=u2[2];
	pid_xun_r.watch_dog=30;
